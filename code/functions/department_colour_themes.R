...
# DECISION: list or vector e.g. theme$Sixth vs theme[6] to reference
# colours i.e. by name or number
colours_dof <- c(
  "#00205B", # banner background
...
colours_teo <- c(
  ...
  "bisque1", "yellow", "black", "pink"
)


# DECISION: how users will choose CSS theme. idea, use this script to
# output .css files and users change the style.css to e.g. style_dof.css
# as required.
...
preview_palette <- function(colours = colours_teo) {
  plot(1:length(colours), 1:length(colours),
    col = colours[1:length(colours)],
    pch = 19, cex = 7,
    xlab = "", ylab = ""
  )
}
