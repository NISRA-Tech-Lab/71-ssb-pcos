...
# nicstheme must be set in config file e.g.:
# nicstheme <- "dfi"
# function will search (default) style.css and find which theme you are using
# i.e. the value of data-nics-theme

# pass the CSS var wanted into the function and it return the value including #
# e.g. if you want --nics-banner-bg value
# get_css_colour(nics-banner-bg)
# note the two dashes (--) at the beginning of the variable should not be
# included


get_css_colour <- function(element_name) {
  # Read in css file
  css_file <- readLines(paste0(here(), "/code/", "style.css"))

  # Line theming for the department starts on
  dept_line <- which(css_file == paste0("[data-nics-theme=", nicstheme, "] {"))
