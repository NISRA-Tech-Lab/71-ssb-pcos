...
f_word_vector <- function(input) {
  ...
  words <- c(
    "[[invalid f_word_vector]]", "[[invalid f_word_vector]]",
    "[[invalid f_word_vector]]", "[[invalid f_word_vector]]",
    "[[invalid f_word_vector]]", "[[invalid f_word_vector]]"
  )
  words <- case_when(
    input < 0 ~ c(
      "decrease", "a decrease", "decreasing", "decreased", "lower",
      "below", "less", "down"
    ),
    input == 0 ~ c("!!NO CHANGE IN VALUE!!"),
    input > 0 ~ c(
      "increase", "an increase", "increasing", "increased",
      "higher", "above", "more", "up"
    ),
...
f_calc_n_diff <- function(current_val, previous_val) {
  as.numeric(current_val) - as.numeric(previous_val)
}
...
f_fix_lgdnames <- function(lgdname) {
  case_when(
    lgdname == "BELFAST" ~ "Belfast",
    lgdname == "Armagh, Banbridge and Craigavon" ~
